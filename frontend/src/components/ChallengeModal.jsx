...
export function ChallengeModal({ isOpen, onClose, claim, onChallengeSubmitted }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-obsidian-base/80 backdrop-blur-sm">
      <div className="bg-whisper-card dark:bg-obsidian-card border border-amber-500/30 rounded-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto shadow-2xl p-6 relative">
        <button
          onClick={onClose}
        ...
        <div className="mb-4 p-3.5 rounded-xl bg-whisper-base dark:bg-obsidian-base border border-emerald-500/20 text-xs flex flex-col gap-1.5">
          <div className="flex items-center justify-between text-emerald-700/70 dark:text-emerald-400/70 font-mono text-[11px]">
            <span>CHALLENGING RECORD #{claim.claim_id || claim.id}</span>
            <span>Target: {claim.entity}</span>
          </div>
          <p className="font-medium text-emerald-950 dark:text-emerald-50 font-sans leading-relaxed">
            "{claim.claim_text}"
          </p>
          <div className="flex items-center gap-2 pt-1 font-mono text-[11px] text-amber-500 font-bold">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <textarea
              ...
              required
              className="w-full text-xs p-3 rounded-lg border border-emerald-500/20 bg-whisper-base dark:bg-obsidian-base text-emerald-950 dark:text-emerald-50 focus:outline-none focus:ring-1 focus:ring-amber-500"
            />
          </div>
                ...
                <div key={index} className="flex items-center gap-2">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-3 py-1.5 text-xs rounded-lg border border-emerald-500/20 bg-whisper-base dark:bg-obsidian-base text-emerald-950 dark:text-emerald-50 focus:outline-none focus:ring-1 focus:ring-amber-500 font-mono"
                    />
                  </div>
          ...
          {/* Counter Bond */}
          <div className="p-3.5 rounded-xl bg-whisper-base dark:bg-obsidian-base border border-amber-500/30 flex flex-col gap-2">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1.5 font-mono text-sm font-bold text-amber-500">
                <input
                  ...
                  className="w-16 px-2 py-1 text-right text-xs rounded border border-amber-500/30 bg-whisper-card dark:bg-obsidian-card font-mono text-emerald-950 dark:text-emerald-50"
                />
                <span>GEN</span>
              </div>
            </div>
          ...
          {isSubmitting ? (
            ...
            <button
              type="submit"
              className="w-full py-2.5 rounded-lg bg-amber-500 hover:bg-amber-600 text-obsidian-base font-bold text-xs shadow transition-colors flex items-center justify-center gap-2"
            >
