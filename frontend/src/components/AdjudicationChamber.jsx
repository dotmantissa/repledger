...
export function AdjudicationChamber({
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-whisper-card dark:bg-obsidian-card border border-emerald-500/20 rounded-xl p-5 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-emerald-700/80 dark:text-emerald-300/80 mt-1 max-w-xl leading-relaxed">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald hover:bg-emerald-600 text-obsidian-base font-bold text-xs transition-colors shrink-0 shadow-sm"
        >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-mono font-medium transition-colors whitespace-nowrap ${
              selectedCategory === cat.id
                ...
                : "bg-whisper-card dark:bg-obsidian-card text-emerald-800 dark:text-emerald-200 border border-emerald-500/15 hover:border-emerald-500/35"
            }`}
          >
            {cat.label}
          </button>
      ...
      {/* Claims Feed */}
      {loading ? (
        <div className="py-16 text-center text-xs font-mono text-emerald-700/60 dark:text-emerald-400/60 flex flex-col items-center gap-2">
        ...
        <div className="bg-whisper-card dark:bg-obsidian-card border border-emerald-500/20 rounded-xl p-12 text-center flex flex-col items-center gap-3">
          ...
          <p className="text-xs text-emerald-700/70 dark:text-emerald-400/70 max-w-md">
          ...
          <button
            onClick={() => onOpenSubmitModal()}
            className="mt-2 px-3.5 py-1.5 rounded-lg bg-emerald hover:bg-emerald-600 text-obsidian-base font-semibold text-xs transition-colors"
          >
            Submit First Claim
          </button>
        </div>
        ...
        <div className="flex flex-col gap-4">
          {filteredClaims.map((c) => {
            ...
            return (
              <div
                key={c.claim_id || c.id}
                className="bg-whisper-card dark:bg-obsidian-card border border-emerald-500/20 rounded-xl overflow-hidden transition-all shadow-sm"
              >
                {/* Structural Device: Consensus Verdict Stamp Top Ribbon */}
                <div className="px-5 py-3 border-b border-emerald-500/15 bg-emerald-500/5 dark:bg-emerald-500/10 flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-xs font-mono font-semibold text-emerald-900 dark:text-emerald-100">
                      #{c.claim_id || c.id}
                    </span>
                    <span className="text-xs font-mono text-emerald-700/60 dark:text-emerald-400/60">
                      • {c.entity}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-3 text-xs font-mono">
                    ...
                    <div className="flex items-center gap-1 px-2 py-0.5 rounded bg-emerald-500/10 text-emerald font-bold">
                ...
                <div className="p-5 flex flex-col gap-3">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="text-sm font-medium text-emerald-950 dark:text-emerald-50 leading-relaxed font-sans">
                        "{c.claim_text}"
                      </p>
                      <div className="flex items-center gap-2 mt-2 text-[11px] font-mono text-emerald-700/70 dark:text-emerald-400/70">
                    ...
                    {isAccepted && (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-amber-500/30 hover:border-amber-500/60 bg-amber-500/10 text-amber-500 hover:bg-amber-500/20 text-xs font-mono font-semibold transition-colors shrink-0"
                      >
                        <Swords className="w-3.5 h-3.5" />
                  ...
                  {isOverridden && (
                    <div className="p-3 rounded-lg bg-blue-500/10 border border-blue-500/20 text-xs text-blue-400 flex items-start gap-2">
                      <Swords className="w-4 h-4 text-blue-400 shrink-0 mt-0.5" />
                      <div>
                  ...
                  <div className="pt-2 border-t border-emerald-500/10 flex items-center justify-between">
                    <button
                      ...
                      className="flex items-center gap-1.5 text-xs font-mono text-emerald hover:underline"
                    >
                      <span>
                        {isExpanded
                    ...
                    {c.genlayer_tx_hash && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="text-[11px] font-mono text-emerald-700/60 dark:text-emerald-400/60 hover:text-emerald flex items-center gap-1"
                      >
                  ...
                  {isExpanded && (
                    <div className="p-4 rounded-lg bg-whisper-base dark:bg-obsidian-base border border-emerald-500/15 flex flex-col gap-3 font-mono text-xs">
                      <div>
                        <span className="text-[10px] text-emerald-700/70 dark:text-emerald-400/70 uppercase">
                          Validator Consensus Summary
                        </span>
                        <p className="text-xs text-emerald-950 dark:text-emerald-100 font-sans mt-0.5 leading-relaxed">
                          {c.consensus_summary ||
                          ...
                          <div className="flex flex-col gap-1 mt-1">
                            {c.evidence_urls.map((url, idx) => (
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="flex items-center gap-1.5 text-emerald hover:underline truncate"
                              >
                      ...
                      {c.key_findings && c.key_findings.length > 0 && (
                        <div>
                          ...
                          <ul className="list-disc list-inside mt-1 flex flex-col gap-0.5 text-emerald-900 dark:text-emerald-200 font-sans text-xs">
                      ...
                      {c.challenges && c.challenges.length > 0 && (
                        <div className="mt-2 pt-2 border-t border-emerald-500/15">
                          ...
                          {c.challenges.map((chal, i) => (
                            <div
                              key={i}
                              className="mt-1.5 p-2 rounded bg-amber-500/5 border border-amber-500/20 text-xs font-sans"
                            >
                              <p className="text-amber-500 font-mono font-bold text-[11px]">
                                Verdict: {chal.verdict} • Counter Bond: {chal.counter_bond} GEN
                              </p>
                              <p className="text-emerald-950 dark:text-emerald-100 mt-1">
                                Rebuttal: "{chal.rebuttal}"
                              </p>
                              {chal.reasoning && (
                                <p className="text-emerald-700/80 dark:text-emerald-300/80 text-[11px] mt-1 font-mono">
